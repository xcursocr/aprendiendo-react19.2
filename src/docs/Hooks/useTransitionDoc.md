...
### 1. El Problema: La Interfaz Congelada 🥶

Imagina que tienes una aplicación con dos pestañas: "Inicio" y "Gráficos Pesados".

1. El usuario está en "Inicio".
2. Hace clic en "Gráficos Pesados".
3. Como renderizar los gráficos toma 2 segundos, **la interfaz se congela**.
4. El usuario intenta hacer clic en "Inicio" para cancelar, pero el botón no responde porque React está bloqueado pintando los gráficos.

### 2. La Solución: `useTransition` 🚦

Este hook te permite marcar un cambio de estado como una **Transición (no urgente)**.
React pintará la nueva pantalla en memoria, y solo cuando esté lista, la mostrará. Mientras tanto, la interfaz vieja sigue respondiendo (es interactiva).
...
### 3. Ejemplo Práctico: Cambio de Pestañas 📑
...
Esta es una mejora gigante. Ahora puedes pasar funciones asíncronas a `startTransition`.
...
const handleSubmit = () => {
  startTransition(async () => {
    // React maneja el estado 'isPending' automáticamente mientras esto ocurre:
...
¡Has recorrido la documentación completa de Hooks de React 19!
