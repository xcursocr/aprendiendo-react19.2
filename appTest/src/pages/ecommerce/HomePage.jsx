 ...
 */
const API_BASE_URL =
  import.meta.env.VITE_API_BASE_URL || "https://gonzapi.domcloud.dev/api/v1";
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "TOGGLE_IN_SET": {
      const { key, value } = action; // key: "brandIds" | "categoryIds" | "colors" | "sizes" | "subcategoryIds"
...
function buildQueryString(f) {
  ...
  if (f.brandIds.size > 0)
    params.set("brand_id", Array.from(f.brandIds).join(","));
  if (f.categoryIds.size > 0)
    params.set("category_id", Array.from(f.categoryIds).join(","));
  if (f.subcategoryIds.size > 0)
    params.set("subcategory_id", Array.from(f.subcategoryIds).join(","));
...
export default function CatalogPage() {
  ...
  const [applied, setApplied] = useState(() =>
    cloneFiltersWithSets(initialFilters)
  );
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const url = `${API_BASE_URL}/${TABLE}?token=false&include=categories,subcategories,brands&${queryString}`;
  ...
  const activeChips = useMemo(() => {
    ...
    if (draft.priceMin !== "" || draft.priceMax !== "")
      chips.push({
        label: `Precio: ${draft.priceMin || "0"} - ${draft.priceMax || "∞"}`,
      });

    for (const id of draft.brandIds) chips.push({ label: `Marca #${id}` });
    for (const id of draft.categoryIds)
      chips.push({ label: `Categoría #${id}` });
    for (const id of draft.subcategoryIds)
      chips.push({ label: `Sub Categoría #${id}` });
...
function FilterSidebar({
  ...
  activeChips,
   ...
   products
    ...
    const brandsApiOption = products.map((b)=>b.brands)
    const categoriesApiOption = products.map((c)=>c.categories)
    const subCategoriesApiOption = products.map((sc)=>sc.subcategories)
    // console.log(brandsApiOption)
  ...
  const subcategoryOptions = [
    { id: "20", id_category: "10", name: "Deportivos" },
    { id: "21", id_category: "11", name: "Blusa" },
    { id: "22", id_category: "12", name: "Abanicos" },
  ];
  ...
  return (
    <aside style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12 }}>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input
            ...
            onChange={(e) =>
              dispatch({ type: "SET_PRICE_MIN", value: e.target.value })
            }
          />
          <input
            style={{ width: "50%" }}
            ...
            onChange={(e) =>
              dispatch({ type: "SET_PRICE_MAX", value: e.target.value })
            }
          />
        </div>
      </div>
      ...
      <div style={{ marginBottom: 12 }}>
        <h4 style={{ margin: "8px 0" }}>Sub Categorías</h4>
        {subCategoriesApiOption.map((sc) => (
          <label key={sc.id} style={{ display: "block" }}>
            <input
              type="checkbox"
              checked={state.subcategoryIds.has(sc.id)}
              onChange={() =>
                dispatch({
                  type: "TOGGLE_IN_SET",
                  key: "subcategoryIds",
        ...
        {colorOptions.map((c) => (
          <label key={c} style={{ display: "block" }}>
            <input
              ...
              onChange={() =>
                dispatch({ type: "TOGGLE_IN_SET", key: "colors", value: c })
              }
            />{" "}
            {c}
          </label>
        ...
        {sizeOptions.map((s) => (
          <label key={s} style={{ display: "block" }}>
            <input
              ...
              onChange={() =>
                dispatch({ type: "TOGGLE_IN_SET", key: "sizes", value: s })
              }
            />{" "}
            {s}
          </label>
  ...
  return (
    ...
    >
      {items.map((p) => (
        ...
        className="relative"
        >
          <div className="bg-white/90 px-4 py-2 h-full">
          ...
          <div style={{ fontSize: 12, opacity: 0.7 }}>
            Sub Category: {p.subcategories.name ?? "-"}
          </div>
          <div style={{ fontSize: 12, opacity: 0.7 }}>
